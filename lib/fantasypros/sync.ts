 ...
 * FantasyPros and upserts them into the shared `players` + `player_rankings`
 * board keyed by (season, scoring). Also enriches `players.draft_year` from
 * Sleeper when available.
 */
export async function syncRankingsForSeason(params: {
  season: number;
  scoring: FpScoring;
}): Promise<SyncRankingsResult> {
  const { season, scoring } = params;
  ...
  try {
    [adpPlayers, rosPlayers, projections] = await Promise.all([
      fetchConsensusRankings({ season, scoring, type: "ADP" }),
      // DRAFT = preseason ECR (full board); ROS can be thinner early in the offseason.
      fetchConsensusRankings({ season, scoring, type: "DRAFT" }),
      fetchProjections({ season, scoring, week: 0 }),
    ]);
  } catch (err) {
  ...
  if (rankingRows.length > 0) {
    const { error } = await admin
      ...
      .upsert(rankingRows, { onConflict: "season,scoring,fp_player_id" });
...
/**
 * Sync the shared rankings board for a draft's season/scoring.
 */
export async function syncRankingsForDraft(draftId: string): Promise<SyncRankingsResult> {
  ...
  return syncRankingsForSeason({
    season: draft.season,
    scoring: draft.scoring as FpScoring,
  });
}
